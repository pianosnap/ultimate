...
export class App {
  ...
  async init(): Promise<void> {
    ...
    // Customization popover bundles theme / particles / chord toggle / background / key width
    this.customizeMenu = new CustomizeMenu(
      ...
      PARTICLE_STYLES,
      {
        ...
        onSetCustomBgColor: (color) => {
          this.store.setCustomBgColor(color)
          this.renderer.setBackgroundColor(color)
          this.customizeMenu.setCustomBgColor(color)
        },
        onSetCustomBgImage: (url) => {
          this.store.setCustomBgImage(url)
          void this.renderer.setBackgroundImage(url)
          this.customizeMenu.setCustomBgImage(url)
        },
        onToggleWhiteKeyLabels: () => {
          const next = !this.store.state.whiteKeyLabels
          this.store.setWhiteKeyLabels(next)
          this.renderer.setShowWhiteKeyLabels(next)
          this.customizeMenu.setWhiteKeyLabels(next)
        },
        onToggleBlackKeyLabels: () => {
          const next = !this.store.state.blackKeyLabels
          this.store.setBlackKeyLabels(next)
          this.renderer.setShowBlackKeyLabels(next)
          this.customizeMenu.setBlackKeyLabels(next)
        },
        onSetKeyRangePreset: (preset) => {
          this.store.setKeyRangePreset(preset)
          this.customizeMenu.setKeyRangePreset(preset)
          let min = 21
          let max = 108
          if (preset === '76') {
            min = 28
            ...
            if (midi) {
              ...
              if (pMin <= pMax) {
                min = Math.max(21, pMin - 2)
                max = Math.min(108, pMax + 2)
              }
            }
          }
          this.renderer.setPitchRange(min, max)
          this.customizeMenu.setVisibleKeysCount(max - min + 1)
        },
        onSetVisibleKeysCount: (count) => {
          this.customizeMenu.setVisibleKeysCount(count)
          const center = 60
        ...
        onTrackColorChange: (trackId, color) => {
          ...
          this.renderer.setTrackColor(trackId, color)
          if (this.store.state.loadedMidi) {
            this.trackPanel.render(this.store.state.loadedMidi)
            this.customizeMenu.setTracks(this.store.state.loadedMidi.tracks)
          }
        },
      },
    ...
    // Apply restored visual customization settings
    if (this.store.state.customBgColor) {
      this.renderer.setBackgroundColor(this.store.state.customBgColor)
      this.customizeMenu.setCustomBgColor(this.store.state.customBgColor)
    }
    if (this.store.state.customBgImage) {
      void this.renderer.setBackgroundImage(this.store.state.customBgImage)
      this.customizeMenu.setCustomBgImage(this.store.state.customBgImage)
    }
    if (this.store.state.whiteKeyLabels) {
      this.renderer.setShowWhiteKeyLabels(true)
      this.customizeMenu.setWhiteKeyLabels(true)
    }
    if (this.store.state.blackKeyLabels) {
      this.renderer.setShowBlackKeyLabels(true)
      this.customizeMenu.setBlackKeyLabels(true)
    }
    this.customizeMenu.setKeyRangePreset(this.store.state.keyRangePreset || '88')

    // Watch loaded MIDI to update track list in CustomizeMenu
    this.unsubs.push(
      watch(
        () => this.store.state.loadedMidi,
        (midi) => {
          this.customizeMenu.setTracks(midi?.tracks ?? [])
        },
      ),
    )
  ...
  private async startExport(settings: ExportSettings): Promise<void> {
    ...
    // Snapshot viewport state so we can restore after export.
    // Ensure keys and bars dimensions are strictly preserved during export
    // unless user explicitly chose a preset or focus mode that demands otherwise.
    ...
    if (framing.pitchRange && this.store.state.keyRangePreset === '88' && settings.focus === 'fit') {
      this.renderer.setPitchRange(framing.pitchRange.min, framing.pitchRange.max)
      pitchChanged = true
    } else {
      // Preserve current key range and bar width exactly
      this.renderer.setPitchRange(originalRange.min, originalRange.max)
    }
