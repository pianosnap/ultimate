...
describe('visual release gate', () => {
  ...
  it.each([
    ...
    'migrates original %s indices without shifting hidden entries',
    (kind, catalog, available, fallback) => {
  ...
  it.each(['lacquer-gold', 'ember-mist'])(
    'preserves deferred saved preference %s when falling back',
    (id) => {
