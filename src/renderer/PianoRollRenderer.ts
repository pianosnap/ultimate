import { Application, Graphics, Sprite, Texture, type Ticker } from 'pixi.js'
...
export class PianoRollRenderer {
  ...
  private customBgColor: number | null = null
  private customBgImage: string | null = null
  private customBgSprite: Sprite | null = null
  ...
  private drawBackground(): void {
    ...
    const bgCol = this.customBgColor !== null ? this.customBgColor : this.theme.background
    g.rect(0, 0, canvasWidth, canvasHeight)
    g.fill({ color: bgCol })

    if (this.customBgSprite && this.customBgTexture) {
      const tw = this.customBgTexture.width || 1
      const th = this.customBgTexture.height || 1
      ...
      this.customBgSprite.width = w
      this.customBgSprite.height = h
      this.customBgSprite.x = (canvasWidth - w) / 2
      this.customBgSprite.y = (canvasHeight - h) / 2
      this.customBgSprite.alpha = 0.85

      // Subtle dimming over the roll area for contrast
  ...
  setBackgroundColor(color: number | string | null): void {
    if (typeof color === 'string') {
      const cleaned = color.trim()
      if (cleaned.startsWith('#')) {
        this.customBgColor = parseInt(cleaned.slice(1), 16)
      } else if (cleaned.length === 6) {
        this.customBgColor = parseInt(cleaned, 16)
      } else {
  ...
  async setBackgroundImage(dataUrl: string | null): Promise<void> {
    this.customBgImage = dataUrl
    if (!dataUrl) {
      if (this.customBgSprite) {
        this.app.stage.removeChild(this.customBgSprite)
        this.customBgSprite.destroy()
        this.customBgSprite = null
      }
      if (this.customBgTexture) {
        this.customBgTexture.destroy(true)
    ...
    try {
      ...
      this.customBgTexture = Texture.from(img)
      if (!this.customBgSprite) {
        this.customBgSprite = new Sprite(this.customBgTexture)
        this.customBgSprite.label = 'custom-background-image'
        const idx = this.app.stage.children.indexOf(this.backgroundGraphics)
        this.app.stage.addChildAt(this.customBgSprite, Math.max(0, idx + 1))
      } else {
        this.customBgSprite.texture = this.customBgTexture
      }
      this.drawBackground()
      this.presentFrame()
    } catch (e) {
  ...
  setShowWhiteKeyLabels(show: boolean): void {
    this.keyboardRenderer.setShowWhiteLabels(show)
    this.presentFrame()
  }

  setShowBlackKeyLabels(show: boolean): void {
    this.keyboardRenderer.setShowBlackLabels(show)
    this.presentFrame()
  }

  get isShowingWhiteKeyLabels(): boolean {
    return this.keyboardRenderer.isShowingWhiteLabels
  }

  get isShowingBlackKeyLabels(): boolean {
    return this.keyboardRenderer.isShowingBlackLabels
  }
