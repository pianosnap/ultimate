...
export class NoteRenderer {
  ...
  setLedGlow(intensity: number): void {
    ...
    if (this.glowFilter) {
      this.glowFilter.innerStrength = this.theme.noteGlowStrength * this.ledGlow * 1.8
      this.glowFilter.outerStrength = Math.min(
        this.theme.noteGlowStrength * this.ledGlow * 0.7,
        4.0,
      )
      this.glowFilter.distance = Math.max(
        4,
        Math.round(this.theme.noteGlowDistance * (0.6 + this.ledGlow * 0.4)),
      )
    }
  }
