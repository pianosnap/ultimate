...
export interface AppStoreState {
  ...
  whiteKeyLabels: boolean
  blackKeyLabels: boolean
...
export function createAppStore() {
  let savedBgColor: string | null = null
  let savedBgImage: string | null = null
  let savedWhiteLabels = false
  let savedBlackLabels = false
  let savedPreset = '88'
  try {
    savedBgColor = localStorage.getItem('midee:customBgColor') || null
    savedBgImage = localStorage.getItem('midee:customBgImage') || null
    savedWhiteLabels = localStorage.getItem('midee:whiteKeyLabels') === 'true'
    savedBlackLabels = localStorage.getItem('midee:blackKeyLabels') === 'true'
    savedPreset = localStorage.getItem('midee:keyRangePreset') || '88'
  } catch {}
  ...
  const [state, setState] = createStore<AppStoreState>({
    ...
    speed: 1,
    transpose: 0,
    customBgColor: savedBgColor,
    customBgImage: savedBgImage,
    whiteKeyLabels: savedWhiteLabels,
    blackKeyLabels: savedBlackLabels,
    keyRangePreset: savedPreset,
  })

  return {
    state,
    setState,
    setCustomBgColor(color: string | null) {
      try {
        if (color) localStorage.setItem('midee:customBgColor', color)
        else localStorage.removeItem('midee:customBgColor')
      } catch {}
      setState('customBgColor', color)
    },
    setCustomBgImage(url: string | null) {
      try {
        if (url) localStorage.setItem('midee:customBgImage', url)
        else localStorage.removeItem('midee:customBgImage')
      } catch {}
      setState('customBgImage', url)
    },
    setWhiteKeyLabels(on: boolean) {
      try {
        localStorage.setItem('midee:whiteKeyLabels', on ? 'true' : 'false')
      } catch {}
      setState('whiteKeyLabels', on)
    },
    setBlackKeyLabels(on: boolean) {
      try {
        localStorage.setItem('midee:blackKeyLabels', on ? 'true' : 'false')
      } catch {}
      setState('blackKeyLabels', on)
    },
    setKeyRangePreset(preset: string) {
      try {
        localStorage.setItem('midee:keyRangePreset', preset)
      } catch {}
      setState('keyRangePreset', preset)
    },
    setTrackColor(trackId: string, color: number) {
      const midi = state.loadedMidi
      if (!midi) return
      const track = midi.tracks.find((t) => t.id === trackId)
      if (track) {
        track.customColor = color
        setState('loadedMidi', { ...midi, tracks: [...midi.tracks] })
      }
    },
